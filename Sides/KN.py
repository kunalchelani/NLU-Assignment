...
def calc_kn_prob(d, ngram):
	...
	if len(ngram) == 1:
		...
		pkn_uni = calc_kn_prob(d, [ngram[0]])
		
		bigram_cnt = get_ngram_cnt(ngram)
		unigram_cnt = pkn_uni*vocab_size
...
def calc_kn_perplexity(d, tokens):

	tot_log_prob = 0.0

	for i in range(0, len(tokens) - 2):
		trigram = tokens[i: i+2]
		prob = calc_kn_prob(d, trigram)
